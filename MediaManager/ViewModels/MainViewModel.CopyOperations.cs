...
/// 
/// ВАЖНО: после обработки последнего элемента очереди ProcessQueueAsync
/// ждёт 300мс и проверяет ещё раз — вдруг за это время пользователь
/// нажал ещё кнопку. Без этого каждый клик запускал отдельный цикл,
/// и вместо очереди «3 из 6» пользователь видел серию одиночных копирований.
/// </summary>
public partial class MainViewModel
{
    ...
    /// <summary>
    /// Задержка перед завершением цикла очереди (мс).
    /// После обработки последнего элемента ждём — вдруг пользователь
    /// нажмёт ещё кнопку. Без этого цикл завершался, _isProcessingQueue
    /// сбрасывался, и следующий клик запускал новый ProcessQueueAsync.
    /// В результате вместо единой очереди «3 из 6» пользователь видел
    /// серию одиночных копирований «по одному».
    /// </summary>
    private const int QueueDrainDelayMs = 300;
    ...
    public async Task ExecuteCopyAsync(MediaFile file, string destinationKey)
    {
        ...
        // Важно: не ждём (await) завершения ProcessQueueAsync!
        // Иначе CopyButton_Click #1 блокировал бы на await до тех пор,
        // пока ВСЯ очередь не обработается, а это мешало бы обработке
        // следующих кликов (клик #2 приходил бы только после завершения #1).
        if (!_isProcessingQueue)
        {
            // Запускаем «в фоне» — fire and forget.
            // Ошибки обрабатываются внутри ProcessQueueAsync (try/finally).
            _ = ProcessQueueAsync();
        }
    }
    ...
    /// 
    /// После обработки последнего элемента ждёт QueueDrainDelayMs (300мс)
    /// и проверяет очередь ещё раз — вдруг пользователь нажал ещё кнопку.
    /// Без этого очередь завершалась между кликами, и каждый клик запускал
    /// отдельный цикл → пропадала шкала прогресса и счётчик «3 из 6».
    /// </summary>
    private async Task ProcessQueueAsync()
    {
        ...
        try
        {
            while (true)
            {
                // Если очередь пуста — ждём немного, вдруг добавят ещё
                if (CopyQueue.Count == 0)
                {
                    // Первая итерация (ничего не обработали) — ждать не нужно,
                    // задача должна вот-вот появиться. Но если мы уже что-то
                    // обработали — пауза даёт пользователю время нажать ещё кнопку.
                    if (_processedCount > 0)
                    {
                        await Task.Delay(QueueDrainDelayMs);

                        // После паузы проверяем ещё раз
                        if (CopyQueue.Count == 0)
                            break; // Очередь действительно пуста — выходим
                    }
                    else
                    {
                        break; // Ничего не обрабатывали и очередь пуста — выходим
                    }
                }

                // Если после паузы появились элементы — продолжаем
                if (CopyQueue.Count == 0)
                    break;
